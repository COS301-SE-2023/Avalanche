...
def categorize(strings,Inputlabels,confidenceMin):
    data = []
    all=strings.split(",")
    classifier = pipeline(task = 'zero-shot-classification', model='./Models/distilbart/', tokenizer='./Models/distilbart/')
    labels=Inputlabels.split(",")
    for x in all:
        #classifier = pipeline("zero-shot-classification",model="valhalla/distilbart-mnli-12-3")
        instance=(classifier(x,candidate_labels=labels,))
        midRes={}
        midRes['domain']=instance['sequence']
        labels=np.array(instance['labels'])
