...
export default function Input({ type, placeholder, name, id, required, disabled, onChange, value, maxLength, error, showLength, pattern }: IInput) {
    ...
    return <>
        <input
            ...
            name={name}
            id={id}
            className={`bg-gray-50 border-2 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-dark-background dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 ${error ? "border-red-500 dark:border-red-600" : "border-gray-300 dark:border-dark-background"}`}
