...
export class QueryBuilderService {
  ...
  async constructQuery(
    ...
    const permissionsForDataSource = userDetails.products.filter(
      (product) => product.dataSource === dataSource,
    );
    ...
    const hasAccess = this.schemaService.validateUserAccess(
      permissionsForDataSource[0].tou,
      query,
      schemaDetail,
    ); // Assuming permission has the required info
